...
const AddEmploy = () => {
    ...
    const validate = () => {
        ...
        if(formData.name.length<5) errors.name = 'Enter valid Name (min:5)'
    ...
    const handleSubmit = async e => {
        ...
        try {
            const response = await axios.post('http://localhost:3000/employs', formData);
            setMessage(response.data.message);
            setFormData({ name: '', employee_id: '', email: '', phone_number: '', department: '', date_of_joining: '', role: '' });
            setError({});
        } catch (err) {
